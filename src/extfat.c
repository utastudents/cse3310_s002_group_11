...
int main()
{
   ...
   int filesize;
   char *src, *dest;
   char inputname[] = "in.txt";
   char outputname[] = "out.txt";

   fp = fopen(outputname, "w+"); // opens the file to write to
   if (fp == -1)
   {
      perror("file error");
      exit(0);
   ...
   fseek(fp, 0, SEEK_END); // goes to the end of file
   fwrite("", 1, 1, fp); // writes one byte at the end of file
   fclose(fp); // closes file

   fr = open(inputname, O_RDONLY); // opens the input file
   if (fr == NULL)
   {
      perror("file error");
      exit(0);
   }
   filesize = lseek(fr, 0, SEEK_END); // size of the file
  
   src = mmap(NULL, filesize, PROT_READ, MAP_PRIVATE, fr, 0); // maps the file to memory
   if (src == MAP_FAILED)
   {
      perror("mmap failed");
      exit(0);
   ...
   fw = open(outputname, O_RDWR | O_CREAT); // opens the output file
   if (fw == NULL)
   {
      perror("file error");
      exit(0);
   ...
   ftruncate(fw, filesize); // presizes the destination file

   dest = mmap(NULL, filesize, PROT_READ | PROT_WRITE, MAP_SHARED, fw, 0); // maps the file to memory
   if (dest == MAP_FAILED)
   {
      perror("mmap failed");
      exit(0);
   ...
   memcpy(dest, src, filesize); // copies the file data from memory into output file
   munmap(src, filesize); 
